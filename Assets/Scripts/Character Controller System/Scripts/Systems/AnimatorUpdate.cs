    ...
    public partial class AnimatorUpdate : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
                ...
                ref CharControllerE control) =>
            {
                ...
                control.TimerForEquipReset = Anim.GetBool(WeaponDrawn) && control.TimerForEquipReset <= 0.0f &&
                                             !Anim.GetCurrentAnimatorStateInfo(0).IsName("Locomotion_Grounded_Weapon0")
                    ? control.EquipResetTimer
                    : Anim.GetCurrentAnimatorStateInfo(0).IsTag("Combo")
                        ? control.EquipResetTimer
                        : control.TimerForEquipReset;
            ...
            Entities.WithoutBurst().WithChangeFilter<CharControllerE>()
                .ForEach((CapsuleCollider capsule, ref CharControllerE Control) =>
                {
        ...
        void HandleGroundedMovement(CharControllerE control, Animator Anim, Rigidbody RB)
        {
            if (!control.Jump)
            {
                ...
                var stickyForceTarget = groundStickyEffect * velocityF * .5f;
                RB.linearVelocity -= Anim.transform.up * stickyForceTarget * SystemAPI.Time.fixedDeltaTime;
            }
            else
            {
        ...
        void UpdateBeast()
        {
            Entities.WithAll<AnimateTag>().WithoutBurst().ForEach((Animator anim, Rigidbody RB, Transform transform,
            ...
            Entities.WithoutBurst().WithChangeFilter<BeastControllerComponent>()
                .ForEach((CapsuleCollider capsule, ref BeastControllerComponent Control) =>
                {
                    capsule.center = Control.CapsuleCenter;
                    capsule.height = Control.CapsuleHeight;
                }).Run();
        ...
        {
            var extraGravityForce = (Physics.gravity * control.m_GravityMultiplier) - Physics.gravity;
            RB.AddForce(extraGravityForce);
