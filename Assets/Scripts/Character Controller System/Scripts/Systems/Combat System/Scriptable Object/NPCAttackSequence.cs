...
namespace DreamersInc.ComboSystem
{
   ...
   public class NPCAttackSequence : ScriptableObject
   {
      ...
      private void ValidateTable(IAttackSequence.AttackType type)
      {
         ...
         foreach (var attack in AttackSequences.Where(attack => attack.Type == type))
         {
      ...
      private IAttackSequence PickAttackSequence(IAttackSequence.AttackType Type)
      {
         ...
         foreach (var attack in AttackSequences.Where(attackSequence => attackSequence.Type == Type)
                     .Where( attackSequence => pickedNumber > attackSequence.ProbabilityRangeFrom && pickedNumber < attackSequence.ProbabilityRangeTo))
         {
            return attack;
         }	
      ...
      {
         return PickAttackSequence(Type).Triggers;
      }
