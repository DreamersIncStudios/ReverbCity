...
namespace Utilities.ReactiveSystem
{
        ...
        where COMPONENT_REACTOR : struct, IComponentReactorTagsForAIStates<COMPONENT, AICOMPONENT>
    {
        ...
        /// <summary>
        /// Query to gather all entities that need to check for change in value.
        /// </summary>
        ...
        protected override void OnUpdate()
        {
            ...
            systemDeps = new ManageComponentRemovalJob()
            {
                ...
                Reactor = _reactor
            }.Schedule(_componentRemovedQuery, systemDeps);
            _entityCommandBufferSystem.AddJobHandleForProducer(systemDeps);
            systemDeps.Complete();
        ...
        public struct ManageComponentAdditionJob : IJobChunk
        {
                ...
                in v128 chunkEnabledMask)
            {
                ...
                for (int i = 0; i < chunk.Count; ++i)
                {
                    // Calls the mathod and reassign the COMPONENT to take into account any modification that may have
                    // accured during the method call.
                    COMPONENT component = components[i];
                    AICOMPONENT AIcomponent = aiComponents[i];
                    Entity entity = entities[i];
                    ...
                    components[i] = component;
                    aiComponents[i] = AIcomponent;
                    // Add the system state component and set it's value that on the next frame, the
                    // ManageComponentValueChangeJob can handle any change in the COMPONENT value.
        ...
        /// <summary>
        /// This system calls the COMPONENT_REACTOR.ComponentRemoved method on all entities
        /// that were strip down of their COMPONENT.
        /// </summary>
        [BurstCompile]
