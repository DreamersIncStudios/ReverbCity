        ...
        protected override void OnCreate()
        {
            patrolStateEntity = GetEntityQuery(new EntityQueryDesc()
            {
                All = new[]
                {
                    ...
                    ComponentType.ReadWrite(typeof(Patrol)), ComponentType.ReadOnly(typeof(LocalToWorld))
                },
            });
            wanderStateEntity = GetEntityQuery(new EntityQueryDesc()
            {
                All = new[]
                {
                    ComponentType.ReadOnly(typeof(SetupBrainTag)),
                    ComponentType.ReadWrite(typeof(WanderQuadrant)), ComponentType.ReadOnly(typeof(LocalToWorld))
                },
            });
            traverseStateEntity = GetEntityQuery(new EntityQueryDesc()
            {
                ...
                {
                    ComponentType.ReadOnly(typeof(TravelWaypointBuffer)), ComponentType.ReadOnly(typeof(SetupBrainTag)),
                    ComponentType.ReadWrite(typeof(Traverse)), ComponentType.ReadOnly(typeof(LocalToWorld))
                },
            });
