...
namespace IAUS.ECS.Systems.Reactive
{
    ...
    public partial class PatrolMovement : SystemBase
    {
        ...
        {
         new PatrolMovementUpdateJob().Schedule(_componentAddedQuery);

        }
        
        [BurstCompile]
        public partial struct PatrolMovementUpdateJob: IJobEntity
        {
            void Execute(ref Patrol patrol, ref Movement move, [ReadOnly] LocalTransform transform)
            {
                
                patrol.StartingDistance = Vector3.Distance( transform.Position, patrol.CurWaypoint.Position);
