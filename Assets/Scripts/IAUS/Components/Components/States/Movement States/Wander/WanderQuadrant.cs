...
namespace IAUS.ECS.Component
{
    public struct WanderQuadrant : IMovementState
    {
        ...
        {
            get { return BufferZone > DistanceToPoint; }
        }
        ...
        public uint NumberOfWayPoints
        {
            get { return 1; }
            set { }
        }

        public float DistanceRatio => (float)DistanceToPoint / (float)StartingDistance != Mathf.Infinity
            ? Mathf.Clamp01((float)DistanceToPoint / (float)StartingDistance)
            : 0;
        ...
        public WanderQuadrant(float3 wanderCenterPoint, float coolDownTime, float bufferZone,
            bool wanderNeighborQuadrants) : this()
        {
