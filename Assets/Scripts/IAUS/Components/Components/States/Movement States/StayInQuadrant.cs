...
namespace IAUS.ECS.Component
{
    public struct StayInQuadrant : IBaseStateScorer
    {
        ...
        {
            get { return AIStates.RetreatToQuadrant; }
        }
    ...
    public readonly partial struct StayInQuadrantAspect : IAspect
    {
        ...
        {
            get { return Vector3.Distance(stay.ValueRO.SpawnPosition, transform.ValueRO.Position); }
        }
        ...
        public float Score
        {
            get
            {
                float totalScore = stay.ValueRO.DistanceToPoint.Output(distanceToPoint) *
                                   stay.ValueRO.HealthRatio.Output(statInfo.ValueRO
                                       .HealthRatio); //TODO Add Back Later * escape.ValueRO.TargetInRange.Output(attackRatio); ;
                stay.ValueRW.TotalScore = stay.ValueRO.Status != ActionStatus.CoolDown && !stay.ValueRO.AttackTarget
                    ? Mathf.Clamp01(totalScore + ((1.0f - totalScore) * stay.ValueRO.mod) * totalScore)
                    : 0.0f;
