...
namespace IAUS.ECS.Component
{
    ...
    public partial struct WaypointCreationSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            foreach (var (buffer, transform, traverse, tag) in SystemAPI.Query<DynamicBuffer<TravelWaypointBuffer>, RefRO<LocalTransform>, Traverse, SetupBrainTag>())
            {
                List<TravelWaypointBuffer> Waypoints = GetPoints(transform.ValueRO.Position, 400, traverse.NumberOfWayPoints, true);
                foreach (var item in Waypoints)
                {
                    buffer.Add(item);
                }
